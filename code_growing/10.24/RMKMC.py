...
from util.logger import AverageMeter, Logger, save_checkpoint
...
def RMKMC(Xs, k, gamma, n_iter = 300, initialization = "rand"):
    ...
    for i in range(n_iter):
        ...
        for v in range(n_views):
            Ftrans = Xs[v].T @ tildeDs[v] @ G @ np.linalg.inv(G.T @ tildeDs[v] @ G)# 这里用python3
            Fs[v] = Ftrans.T
...
if __name__ == '__main__':
    ...
    ind_matrix, Fs, aa = RMKMC(X, 8, 1.2, n_iter=2, initialization="rand")
